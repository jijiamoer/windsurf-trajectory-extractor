...
def cmd_ag_list(
    ...
    """List Antigravity conversations using both summaries and local artifacts."""
    conversations = list_antigravity_conversations(
        state_db, conversations_dir, brain_dir
    )
    if not conversations:
        print("No Antigravity conversations found.", file=sys.stderr)
        return 1

    print(
        f"{'Blob MB':>8} {'Cascade ID':>36}  {'S':>1} {'B':>1}  {'Title / Workspace'}"
    )
    print("-" * 120)
    for item in conversations:
        size_mb = (
            f"{item['conversation_blob_size'] / 1024 / 1024:.1f}"
            if item["conversation_blob_size"]
            else "-"
        )
        flags = (
            "Y" if item["has_summary"] else "-",
            "Y" if item["has_brain_dir"] else "-",
        )
        label = item["title"] or (
            item["workspace_paths"][0] if item["workspace_paths"] else ""
        )
        print(
            f"{size_mb:>8} {item['cascade_id']}  {flags[0]:>1} {flags[1]:>1}  {label}"
        )
    return 0
...
def cmd_ag_extract(
    ...
    """Extract Antigravity metadata for one conversation."""
    try:
        result = extract_antigravity_conversation(
            cascade_id, state_db, conversations_dir, brain_dir
        )
    except KeyError as e:
        print(f"Error: {e}", file=sys.stderr)
        return 1
    except (OSError, sqlite3.Error, ValueError, binascii.Error) as e:
        print(f"Error extracting Antigravity conversation: {e}", file=sys.stderr)
        return 1
    ...
    if output:
        try:
            with open(output, "w", encoding="utf-8") as f:
                ...
                f.write("\n")
            print(f"Wrote Antigravity metadata to {output}", file=sys.stderr)
        except OSError as e:
            print(f"Error writing output file: {e}", file=sys.stderr)
            return 1
    else:
        summary = result.get("summary") or {}
        blob = result.get("conversation_blob") or {}
        brain = result.get("brain") or {}
        print(f"Cascade ID: {cascade_id}")
        if summary.get("title"):
            print(f"Title: {summary['title']}")
        if summary.get("workspace_paths"):
            print(f"Workspace: {summary['workspace_paths'][0]}")
        if summary.get("last_modified"):
            print(f"Summary modified: {summary['last_modified']}")
        if blob:
            print(
                f"Blob: {blob['size_bytes']} bytes, modified {blob['modified_at']}, head {blob['header_hex']}"
            )
        if brain:
            print(f"Brain files: {len(brain.get('files', []))}")
            for item in brain.get("files", [])[:10]:
                print(f"  - {item['path']} ({item['size_bytes']} bytes)")
            remaining = len(brain.get("files", [])) - 10
            if remaining > 0:
...
Examples:
  ...
  %(prog)s --ag-list                   List Antigravity conversations
  %(prog)s --ag-extract ID -o out.json Extract Antigravity metadata
...
Features:
  ...
  - Antigravity summary/blob/brain correlation
""",
    )
    parser.add_argument("--list", action="store_true", help="List all workspaces")
    parser.add_argument(
        "--ag-list", action="store_true", help="List Antigravity conversations"
    )
    parser.add_argument(
        "--ag-extract", metavar="CASCADE_ID", help="Extract Antigravity metadata"
    )
    parser.add_argument(
    ...
    if args.ag_list or args.ag_extract:
        state_db, conversations_dir, brain_dir = find_antigravity_paths()
        if state_db is None and conversations_dir is None and brain_dir is None:
            print("Error: Antigravity installation not found.", file=sys.stderr)
            sys.exit(1)

        if args.ag_list:
            sys.exit(cmd_ag_list(state_db, conversations_dir, brain_dir))
        sys.exit(
            cmd_ag_extract(
                args.ag_extract,
                args.output,
